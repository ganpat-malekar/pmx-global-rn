...
function CompanyShareholders(props) {
  ...
  const handleDelete = async (popupState, DocumentDetailId, rowIndex) => {
    ...
    if (shouldCallApi) {
      ...
      success && props.deleteCompanyShareholderRecord(rowIndex);
    } else {
      props.deleteCompanyShareholderRecord(rowIndex);
    }
  ...
  const columns = [
    {
      name: "Name",
      label: "Name",
      options: { ...tableColumnOptions, sort: false },
    },
    {
      name: "DocumentType",
      label: "Document Type",
      options: {
        ...
        customBodyRender: (value, tableMeta, updateValue) => {
          // let temp = props.CompanyDocumentNames.filter((e) => e.Text === value);
          // return temp[0].Text;
          return value;
        },
      },
            ...
            {props.apiStatus === null ? (
              ...
              <>
                ...
                <div className={classes.NewDataTable}>
                  <MUIDataTable
                    data={
                      !_.isEmpty(props.CompanyShareholders)
                        ? props.CompanyShareholders.map((item, index) => {
                            return {
                              Name: item.CompanyName,
                              DocumentType: item.DocumentName,
                              Document: item.DocumentFileName,
                              Country: item.CountryId,
                              DOI: item.DateOfIncorporation,
                              IssueDate: item.DocumentIssueDate,
