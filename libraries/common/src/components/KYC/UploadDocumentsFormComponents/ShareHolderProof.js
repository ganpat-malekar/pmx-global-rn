    ...
    if (validator1.allValid()) {
      if (files.commonFile?.name) {
        ...
        try {
          ...
          console.log(data);
          const success = await props.saveShareHolderData(data);
          if (success) {
            if (props.fromPage === "AddContact") {
              history.push("/App/ManageContacts");
            } else if (props.fromPage === "AddBusiness") {
              history.push("/App/ManageBusiness");
            }
          }
        } catch (err) {
          console.log(err);
